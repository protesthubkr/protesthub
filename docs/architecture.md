...
```text
src/app/
  page.tsx                       공개 캘린더 route. 월 요약 초깃값을 클라이언트 shell에 전달한다.
  list/page.tsx                  공개 목록 route. 1주 window 초깃값을 클라이언트 shell에 전달한다.
...
src/features/public-events/
  ...
  filters.ts                     URL query 파싱, 조건 칩, `/`와 `/list` 이동 URL 생성.
...
src/lib/telegram/
  ...
  channel-candidate-ingest.ts    텔레그램 후보/media 저장 순서만 조율.
  channel-candidate-evaluation.ts 텔레그램 메시지의 후보 생성/검수 상태 판정.
  channel-candidate-rows.ts      review_candidates insert row 생성.
  channel-candidate-repository.ts 후보 insert/update와 자동 승격 저장.
  channel-candidate-media.ts     텔레그램 메시지 이미지 media 저장.
  channel-candidate-keys.ts      source_record_id/media_key 생성.
  channel-candidate-review.ts    review_reason 병합과 보호된 결정 판정.
...
src/lib/x-ingest/
  ...
  x-api-client.ts                X API 공통 fetch/retry와 XApiError.
  x-api-fields.ts                X API field/expansion/query 상수.
  x-api-following.ts             운영 계정 following 목록 조회.
  x-api-tweets.ts                timeline, 단건, 다건 tweet 조회.
  config.ts                      환경변수 파싱.
...
1. `src/app/page.tsx`는 캘린더 뷰(`/`)를, `src/app/list/page.tsx`는 리스트 뷰(`/list`)를 담당하고 둘 다 `getPublicEventsHomePageData()`를 호출해 route를 얇게 유지한다.
2. `home-page-data.ts`가 route view mode와 `searchParams`를 `parseEventSearchState()`로 변환한다.
...
8. 공개 후 `/`, `/list`, `/events/[id]`, `/admin/candidates`를 revalidate한다. `/api/events`는 짧은 CDN TTL로 최대 60초 안에 갱신된다.
...
5. 신규 채널처럼 탐색 기록이 없으면 첫 수집에서 최대 60일 전까지 확인한다. 기본 동작은 날짜 cutoff까지 탐색하고, `TELEGRAM_CHANNEL_SCAN_MAX_PAGES`가 설정된 경우에만 운영자가 지정한 페이지 상한을 추가 안전장치로 적용한다. 이미 수집한 채널은 `last_checked_message_id` 이후 메시지만 후보화한다.
...
7. 승격 기준을 만족하지 못하거나 오늘 이전 일정으로 판정된 메시지는 `ignored`로 저장한다. 기존 자동 후보가 있으면 새 기준의 `needs_review` 판정으로 승격하거나 이미지/본문/근거를 병합하되, `admin_*`, `manual_*`, `published_event`, `unpublished_event` 근거가 있거나 `duplicate`/`canceled`/`published` 상태인 후보는 자동으로 덮어쓰지 않는다.
...
3. 팔로잉 목록을 새로 반영해야 할 때는 `/admin/candidates`의 X 수집 실행 패널에서 "팔로잉 갱신 후 수집"을 누른다. API로 직접 실행해야 할 때만 `/api/ingest/x?refreshFollowing=true`를 사용한다. X API가 팔로잉 목록 전체를 반환한 실행에서는 더 이상 팔로우하지 않는 기존 계정을 `is_following=false`로 내린다. `X_MAX_FOLLOWING_ACCOUNTS` 제한 때문에 목록이 잘렸다면 언팔로우 반영은 하지 않는다.
...
6. 공식 계정의 리포스트 wrapper는 후보로 만들지 않는다. 대신 `referenced_tweets.type=retweeted` 원포스트 id만 모아 `/2/tweets?ids=...`로 원포스트를 hydrate하고 후보화한다. 원포스트 작성자가 현재 DB의 전체 `is_following=true` 캐시에 있으면 후보화하지 않는다.
...
17. 계정별 timeline 조회가 끝나면 즉시 `x_accounts.last_ingested_at`, `last_ingested_post_id`, `last_ingested_post_created_at`, `last_ingest_run_id`를 갱신한다. 신규 계정에 post가 없어도 `last_ingested_at`은 남겨 다음 수집에서 같은 30일 범위를 반복 조회하지 않는다. 같은 초에 여러 post가 있으면 post id까지 비교해 최신 cursor를 고른다.
...
- 후보 기준을 바꿀 때는 `src/lib/x-ingest/normalize-rules.ts`, `normalize-signals.ts`, `normalize-keywords.ts`, `candidate-rows.ts`를 함께 본다.
