export function formatCandidateReason(reason: string) {
  ...
  if (reason.startsWith("notice_hint:")) {
    return `공지형 신호 ${reason.replace("notice_hint:", "")}`;
  }
  ...
  if (reason.startsWith("review_suppressed:")) {
    ...
    return rule === "notice_only" ? "검수 억제 공지만 있음" : `검수 억제 ${rule}`;
  }
  ...
  if (reason.startsWith("llm_fallback_from:")) {
    return `LLM 재시도 이전 모델 ${reason.replace("llm_fallback_from:", "")}`;
  }
  ...
  const exactLabels: Record<string, string> = {
    ...
    manual_telegram_link: "텔레그램 링크 수동 추가",
    ...
    x_detail_deferred: "X 상세 수집 대기",
    x_detail_hydrated: "X 상세 수집 완료",
  };

  return exactLabels[reason] ?? reason;
}
